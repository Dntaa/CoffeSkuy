...
<body class="bg-[#1c1b19] text-gray-100">
  ...
  <header class="bg-[#ece0d1] text-[#1c1b19] py-16 text-center">
    ...
    <p class="mt-3 text-lg text-gray-700">From fresh brew to sweet bites — we’ve got your cravings covered.</p>
  </header>
    ...
    <div class="max-w-screen-xl mx-auto">
      <div class="grid md:grid-cols-3 sm:grid-cols-2 gap-8">
        ...
        @forelse ($menus as $menu)
          <div class="bg-[#2d2b28] rounded-xl overflow-hidden shadow-md border border-[#3a3734]">
            <img src="{{ asset('img/' . $menu->gambar) }}" alt="{{ $menu->nama }}" class="w-full h-48 object-cover">
            <div class="p-5">
              <h2 class="text-xl font-bold text-white mb-1">{{ $menu->nama }}</h2>
              ...
              <div class="flex items-center justify-between">
                ...
                <a href="{{ url('/pesan/' . urlencode($menu->nama)) }}"
                   class="bg-amber-500 hover:bg-amber-600 text-white px-4 py-2 rounded-full text-sm transition-all">
                  Pesan
                </a>
              </div>
            </div>
